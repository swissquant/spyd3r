...
@to_async
def fetch_oracle_value(address: str, decimals: int = 8):
    contract = get_contract(address=address)
    dt = contract.functions.latestRoundData().call()
    price = dt[1] / 10**decimals

    return price


async def fetch_price(market: str, oracles: dict[str, str]):
    address = oracles.get(market, "")
    if address != "":
        return await fetch_oracle_value(address=address)
    else:
        return 0.0


@to_async
def fetch_aggregator_address(address: str, decimals: int = 8):
    contract = get_contract(address=address)
